...
@section('title', 'Edit Astrologer Review')
@section('content')
<div class="card mb-4">
    <div class="card-body">
        <h5 class="mb-3">Edit Review</h5>
        <form method="POST" action="{{ route('admin.astrologer_reviews.update', $review->id) }}">
            @csrf
            @method('PATCH')
            <div class="mb-3">
                ...
                <select name="astrologer_id" id="astrologer_id" class="form-control @error('astrologer_id') is-invalid @enderror" required>
                    ...
                    @foreach($astrologers as $astrologer)
                        <option value="{{ $astrologer->id }}" {{ old('astrologer_id', $review->astrologer_id) == $astrologer->id ? 'selected' : '' }}>
            ...
            <div class="mb-3">
                ...
                <textarea name="review" id="review" rows="4" class="form-control @error('review') is-invalid @enderror">{{ old('review', $review->review) }}</textarea>
